...
serve(async (req) => {
  ...
  try {
    ...
    // Call Ideogram API to generate 4 covers
    console.log("Calling Ideogram API for 4 covers...");
    
    const imagePromises = [];
    for (let i = 0; i < 4; i++) {
      const imagePromise = fetch('https://api.ideogram.ai/generate', {
        method: 'POST',
        headers: {
          'Api-Key': ideogramKey,
    ...
    for (let i = 0; i < responses.length; i++) {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Ideogram API error for image ${i + 1}: ${response.status} - ${errorText}`);
      }
      
      const data = await response.json();
      console.log(`Ideogram response ${i + 1} received:`, data.data?.length || 0, "images");
